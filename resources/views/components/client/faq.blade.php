<!-- ======= F.A.Q Section ======= -->
<div class="mt-4">
    <div class="container mb-4">

        <!-- ======= F.A.Q Section ======= -->
        <div class="mt-4">
            <div class="container mb-4">
                <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
                    <h1 class="mb-3">Persyaratan Pendaftaran</h1>
                    <ul class="list-unstyled">
                        <li>• Mengisi Formulir Pendaftaran</li>
                        <li>• Mengunggah SKL</li>
                        <li>• Mengunggah Akta Kelahiran</li>
                        <li>• Mengunggah Kartu Keluarga</li>
                        <li>• Mengunggah Kartu Nomor Induk Siswa Nasional (NISN)</li>
                        <li>• Mengikuti tes Akademik, Wawancara dan Baca Al Qur'an</li>
                    </ul>
                </div>




                <div class="row g-2">
                    <div class="col-lg-6">
                        <!-- F.A.Q List 1-->
                        <div class="accordion accordion-flush" id="faqlist1">
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq-content-1">
                                        Bagaimana cara mendaftar?
                                    </button>
                                </h2>
                                <div id="faq-content-1" class="accordion-collapse collapse" data-bs-parent="#faqlist1">
                                    <div class="accordion-body">
                                        Caranya cukup mudah dengan cara klik tombol daftar sekarang yang ada di pojok
                                        kanan
                                        atas atau klik <a class="scrollto" href="{{route('daftar')}}"
                                            target="_blank">disini<i class="ri-arrow-right-up-line"></i></a>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq-content-2">
                                        Apakah daftarnya wajib online?
                                    </button>
                                </h2>
                                <div id="faq-content-2" class="accordion-collapse collapse" data-bs-parent="#faqlist1">
                                    <div class="accordion-body">
                                        Wajib online, tapi boleh offline bagi yang berkenan datang langsung ke sekolah

                                    </div>
                                </div>
                            </div>

                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq-content-3">
                                        Apakah tesnya boleh online?
                                    </button>
                                </h2>
                                <div id="faq-content-3" class="accordion-collapse collapse" data-bs-parent="#faqlist1">
                                    <div class="accordion-body">
                                        Untuk tes masuk wajib datang langsung ke sekolah
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="col-lg-6">
                        <!-- F.A.Q List 2-->
                        <div class="accordion accordion-flush" id="faqlist2">
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq2-content-1">
                                        Dimana Lokasi MTs SA Darul Muqodas?
                                    </button>
                                </h2>
                                <div id="faq2-content-1" class="accordion-collapse collapse" data-bs-parent="#faqlist2">
                                    <div class="accordion-body">
                                        Banger, Mojomulyo, Kec. Tambakromo, Kabupaten Pati, Jawa Tengah
                                    </div>
                                </div>
                            </div>

                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq2-content-2">
                                        Apa saja fasilitas yang disediakan?
                                    </button>
                                </h2>
                                <div id="faq2-content-2" class="accordion-collapse collapse" data-bs-parent="#faqlist2">
                                    <div class="accordion-body">
                                        Untuk melihat informasi dapat dilihat
                                        <a href="{{route('blog')}}" target="_blank">disini<i
                                                class="ri-arrow-right-up-line"></i></a>
                                    </div>
                                </div>
                            </div>

                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed wow fadeInUp" data-wow-delay="0.3s"
                                        type="button" data-bs-toggle="collapse" data-bs-target="#faq2-content-3">
                                        Bagaimana sistem pembelajaran di MTs SA Darul Muqodas?
                                    </button>
                                </h2>
                                <div id="faq2-content-3" class="accordion-collapse collapse" data-bs-parent="#faqlist2">
                                    <div class="accordion-body">
                                        Kami menggunakan Kurikulum Merdeka yang terintegrasi dengan pendidikan
                                        agama Islam, serta berbasis karakter dan literasi
                                        digital.
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            </section>
            <!-- End F.A.Q Section -->