...
class ClientController extends Controller
{
    ...
    public function yes_daftar(Request $req)
    {
        ...
        $kk = $req->file('upload_kk')->store('upload/kk', 'public');
        $nisn = $req->file('upload_nisn')->store('upload/nisn', 'public');
        $akta = $req->file('upload_akta')->store('upload/akta', 'public');
        $skl = $req->file('upload_skl')->store('upload/skl', 'public');

        $last = Pendaftar::orderBy('id', 'desc')->first();
        $nextId = $last ? $last->id + 1 : 1;
        $nomor_pendaftaran = 'REG-' . date('Ymd') . '-' . str_pad($nextId, 5, '0', STR_PAD_LEFT);
